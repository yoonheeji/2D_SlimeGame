...
{
    public float rewardInterval { get; set; } = 10f; // 보상 간격 (초 단위)
                                                     //public int rewardAmount { get; set; } = 100; // 보상 양
    ...
    IEnumerator CoStartTimer()
    {
        while (true)
        {
            ...
            //Debug.Log("Elapsed time: " + formattedTime);
    ...
    public void CheckAttendance()
    {
        if (IsSameDay(LastLoginTime, DateTime.Now) == false)
        {
            ...
            Managers.Game.GachaCountAdsAnvanced = 1;
    ...
    public void GiveFastOfflineReward(OfflineRewardData data)
    {
        ...
        if (Managers.Game.DicMission.TryGetValue(MissionTarget.OfflineRewardGet, out MissionInfo mission))
            mission.Progress++;
